...
training_in_file = open(r"Sets\training_in_8.pkl","rb")
training_in_db = pickle.load(training_in_file)
training_in_file.close()
training_out_file = open(r"Sets\training_out_8.pkl","rb")
...
testing_in_file = open(r"Sets\testing_in_8.pkl","rb")
testing_in_db = pickle.load(testing_in_file)
testing_in_file.close()
testing_out_file = open(r"Sets\testing_out_8.pkl","rb")
...
for i in range(len(testing_in_df)):
    ...
    if labels_df.loc[ind,'label'] == '4':
        ptcolor = "coral"
    if labels_df.loc[ind,'label'] == '5':
            ptcolor = "lightcoral"
    if labels_df.loc[ind,'label'] == '6':
        ptcolor = "palegreen"
    if labels_df.loc[ind,'label'] == '7':
        ptcolor = "maroon"
    if labels_df.loc[ind,'label'] == '8':
        ptcolor = "darkgreen"
...
for i in range(len(testing_in_df)):
    ...
    if linsvc.predict(testing_in)[i] == '4':
        ptcolor = "coral"
    if linsvc.predict(testing_in)[i] == '5':
            ptcolor = "lightcoral"
    if linsvc.predict(testing_in)[i] == '6':
        ptcolor = "palegreen"
    if linsvc.predict(testing_in)[i] == '7':
        ptcolor = "maroon"
    if linsvc.predict(testing_in)[i] == '8':
        ptcolor = "darkgreen"
...
for i in range(len(testing_in_df)):
    ...
    if svclink.predict(testing_in)[i] == '4':
        ptcolor = "coral"
    if svclink.predict(testing_in)[i] == '5':
            ptcolor = "lightcoral"
    if svclink.predict(testing_in)[i] == '6':
        ptcolor = "palegreen"
    if svclink.predict(testing_in)[i] == '7':
        ptcolor = "maroon"
    if svclink.predict(testing_in)[i] == '8':
        ptcolor = "darkgreen"
    x = v3features_df.iloc[ind,0]
    y = v3features_df.iloc[ind,1]
...
for i in range(len(testing_in_df)):
    ...
    if svcrbfk.predict(testing_in)[i] == '4':
        ptcolor = "coral"
    if svcrbfk.predict(testing_in)[i] == '5':
            ptcolor = "lightcoral"
    if svcrbfk.predict(testing_in)[i] == '6':
        ptcolor = "palegreen"
    if svcrbfk.predict(testing_in)[i] == '7':
        ptcolor = "maroon"
    if svcrbfk.predict(testing_in)[i] == '8':
        ptcolor = "darkgreen"
    x = v3features_df.iloc[ind,0]
    y = v3features_df.iloc[ind,1]
...
for i in range(len(testing_in_df)):
    ...
    if svcpolyk.predict(testing_in)[i] == '4':
        ptcolor = "coral"
    if svcpolyk.predict(testing_in)[i] == '5':
            ptcolor = "lightcoral"
    if svcpolyk.predict(testing_in)[i] == '6':
        ptcolor = "palegreen"
    if svcpolyk.predict(testing_in)[i] == '7':
        ptcolor = "maroon"
    if svcpolyk.predict(testing_in)[i] == '8':
        ptcolor = "darkgreen"
    x = v3features_df.iloc[ind,0]
    y = v3features_df.iloc[ind,1]
...
ax.legend(handles = [spine_patch, shaft_patch, soma_patch, ais_patch, partialspine_patch, partialshaft_patch, mergedspine_patch, mergedshaft_patch], fontsize = 'x-small')
ax2.legend(handles = [spine_patch, shaft_patch, soma_patch, ais_patch, partialspine_patch, partialshaft_patch, mergedspine_patch, mergedshaft_patch], fontsize = 'x-small')
ax3.legend(handles = [spine_patch, shaft_patch, soma_patch, ais_patch, partialspine_patch, partialshaft_patch, mergedspine_patch, mergedshaft_patch], fontsize = 'x-small')
ax4.legend(handles = [spine_patch, shaft_patch, soma_patch, ais_patch, partialspine_patch, partialshaft_patch, mergedspine_patch, mergedshaft_patch], fontsize = 'x-small')
ax5.legend(handles = [spine_patch, shaft_patch, soma_patch, ais_patch, partialspine_patch, partialshaft_patch, mergedspine_patch, mergedshaft_patch], fontsize = 'x-small')
